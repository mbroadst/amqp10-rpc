...
RpcServer.prototype._processMessage = function(receiver, message) {
  ...
  if (typeof message.body === 'string') {
    try {
      ...
      return this._respond(replyTo, correlationId, { error: new errors.ParseError(err.message, message.body) });
    }
  } else {
  ...
  var self = this;
  var response = Promise.try(function() { return self._processRequest(replyTo, correlationId, request); });
  return response
    .then(function(response) {
      if (response === undefined) response = null;
      var isError = (response && response.hasOwnProperty('error'));
      var result =  isError ? response : { result: response };
      return self._respond(replyTo, correlationId, result);
    })
    .error(function(err) { self._logger.error(err); })
    .catch(function(err) {
      if ((replyTo === null || replyTo === undefined &&
          correlationId === null || correlationId === undefined) ||
          !(err instanceof errors.ProtocolError)) {
        return self._logger.error(err);
      }

      return self._respond(replyTo, correlationId, { error: err });
    });
};
