...
RpcServer.prototype._processMessage = function(receiver, message) {
  ...
  if (typeof message.body === 'string') {
    try {
      ...
      return this._error(replyTo, correlationId, errors.ParseError, err.message);
    }
  } else {
  ...
  if (!controlMessage.hasOwnProperty('method')) {
    return this._error(replyTo, correlationId, errors.InvalidRequest, 'Missing required property: method');
  }

  var method = controlMessage.method,
      params = controlMessage.params || [];
  if (!this._methodHandlers.hasOwnProperty(method)) {
    return this._error(replyTo, correlationId, errors.MethodNotFound, 'No such method: ' + method);
  }
